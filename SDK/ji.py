...
#     res = res[out_blob]
#     data = res
#     data=data.reshape(data.shape[0],-1,data.shape[4])
#     data = non_max_suppression(data)

#     detect_objs = []
#     for proposal in enumerate(data):
#         if proposal[2] > 0:
#             #batch_id = np.int(proposal[0])
#             label = np.int(proposal[5])
#             confidence = proposal[4]
#             xmin = np.int(iw * proposal[0])
#             ymin = np.int(ih * proposal[1])
#             xmax = np.int(iw * proposal[2])
#             ymax = np.int(ih * proposal[3])
#             if label not in label_id_map:
#                 log.warning(f'{label} does not in {label_id_map}')
#                 continue
#             if proposal[4] > thresh:
#                 detect_objs.append({
...
# if __name__ == '__main__':
#     # Test API
#     img = cv2.imread('/project/train/images/train2017/1014a144.jpg')
#     predictor = init()
